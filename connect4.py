...
class Joc:
    ...
    def final(self):
        ...
        for linie_vals in self.matr:
            for coloana_idx in range(len(linie_vals)-3):
                curent = linie_vals[coloana_idx]
                if curent != '#':
                    ok = 1
                    for col_verif in range(coloana_idx+1,coloana_idx+4):
                        if curent != linie_vals[col_verif]:
                            ok = 0
                    if ok == 1:
                        return curent
        ...
        for j in range(self.NR_COLOANE):
            for i in range(self.NR_LINII-3):
                curent = self.matr[i][j]
                if curent != '#':
                    ok = 1
                    for k in range(i+1,i+4):
                        if curent != self.matr[k][j]:
                            ok = 0
                    if ok == 1:
                        return curent
        ...
        for lin in range(self.NR_LINII):
            for col in range(self.NR_COLOANE):
                diag1 = self.get_diagonala1(lin,col)
                diag = set(diag1)
                if len(diag) == 1 and diag1[0] != '#':
                    return diag1[0]

        # diagonale /
        for lin in range(self.NR_LINII):
            for col in range(self.NR_COLOANE):
                diag1 = self.get_diagonala2(lin,col)
                diag = set(diag1)
                if len(diag) == 1 and diag1[0] != '#':
                    return diag1[0]
    ...
    def nr_intervale_deschise(self, jucator):
        ...
        for linie_vals in self.matr:
            for coloana_idx in range(len(linie_vals)-3):
                lista_vals = []
                for col_verif in range(coloana_idx,coloana_idx+4):
                    lista_vals.append(linie_vals[col_verif])
                if juc_opus not in lista_vals:
                    rez +=1
        ...
        for j in range(self.NR_COLOANE):
            for i in range(self.NR_LINII-3):
                ...
                for k in range(i,i+4):
                    col_vals.append(self.matr[k][j])
                if juc_opus not in col_vals:
                    rez += 1
